...
Factor features a fully featured development framework. It can be used stand-alone in similar to use cases to a more typical meta framework (e.g. Nuxt); but it also serves as the basis for building data management features as well as the extension API.  The framework includes: 
