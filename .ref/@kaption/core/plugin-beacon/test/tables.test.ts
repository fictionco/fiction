...
import type { SessionParams } from '../../plugin-beacon'
...
describe('standard tables', () => {
  ...
  it('runs correctly with subquery', async () => {
    const { kaptionClickHouse } = testUtils

    if (!kaptionClickHouse)
      throw new Error('no clickhouse')

    await saveKaptionEvents(testUtils)

    const cli = kaptionClickHouse.client()

    const base = cli.from(kaptionClickHouse.tableEvents)

    const query = kaptionClickHouse
      .sessionTable({ base })
      .orderBy('session_endedAt', 'desc')
      .whereRaw(
        `toYYYYMMDDhhmmss(session_timestamp) > ${kaptionClickHouse.formatTime(
    ...
    const { data } = await kaptionClickHouse.clickHouseSelect<SessionParams[]>(
      query,
    )
