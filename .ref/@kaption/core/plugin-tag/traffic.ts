...
import { ANON_ID_KEY } from '@kaption/client/config'
import type { FactorTestingApp } from '@factor/api/plugin-testing-app'
import { ReferrerUtility } from '../plugin-beacon/utils/referrer'

type KaptionTrafficGeneratorSettings = {
  factorTestingApp: FactorTestingApp
  isLive?: vue.Ref<boolean>
} & FactorPluginSettings
export class KaptionTrafficGenerator extends FactorPlugin<KaptionTrafficGeneratorSettings> {
  ...
  dayOfWeek = this.utils.dayjs().day()
  speed = this.dayOfWeek > 4 ? 30 : 50
  maxActions = this.dayOfWeek > 4 ? 14 : 20
  minActions = 1
  contextCount = 0
  lastRunAt?: dayjs.Dayjs
  constructor(settings: KaptionTrafficGeneratorSettings) {
  ...
  async newFakeUser(params: { entryUrl: string }) {
    ...
    const anonymousId = maybeReturning
      ? `returning-${this.utils.randomBetween(1, 10)}`
      : this.utils.objectId()
            ...
            cookies: [
              {
                ...
                sameSite: 'Lax',
                expires: this.utils.dayjs().add(365, 'day').unix(),
                secure: false,
                httpOnly: false,
              },
            ],
  ...
  async visit(params: { entryUrl: string }) {
    ...
    try {
      ...
      this.lastRunAt = this.utils.dayjs()
    }
    catch (error) {
      this.log.warn(
  ...
  async run(params: { entryUrl: string }) {
    ...
    if (this.isLive?.value) {
      ...
      this.intervalId = setInterval(async () => {
        if (this.lastRunAt) {
          const diff = Math.abs(
            this.utils.dayjs().diff(this.lastRunAt, 'minute'),
          )
          if (diff > 5) {
            this.log.error(
