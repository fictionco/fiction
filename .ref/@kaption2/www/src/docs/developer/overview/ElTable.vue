  ...
  fieldsList.value.push(
    {
      ...
      type: 'boolean',
      description:
        'Whether an update was created by an active user. Set to false to prevent \'lastSeenAt\' from being updated.',
    },
    ...
    },
    {
      ...
      mandate: 'set implicitly',
      type: 'object',
      description: 'Object describing the how a user was referred to the app.',
    ...
    },
    {
      ...
      mandate: 'set implicitly',
      type: 'object',
      description: 'Object describing the Kaption API library.',
      keys: ['name', 'version'],
    },
  )
}
  ...
  fieldsList.value.push(
    {
      ...
      type: 'string',
      description:
        'Unique id associated with the user in your database. If omitted anonymousId must be set and is set automatically by Kaption in the browser.',
    },
    {
      property: 'traits',
      mandate: 'optional',
...
if (props.fields === 'traits') {
  fieldsList.value.push(
    ...
    },
    {
      property: 'createdAt',
      type: 'date',
      description: 'Date when the user account was created. ISO-8601 format.',
    },
  )
}
  ...
  <div class="my-12 flex flex-col">
    <div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
      <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
        <div class="overflow-hidden border border-slate-100 sm:rounded-lg">
          <table class="min-w-full divide-y divide-slate-100 rounded-md">
            <thead class="bg-theme-50 text-theme-500">
              <tr>
                <th
                  scope="col"
            ...
            <tbody>
              <tr
                ...
                :key="i"
                :class="i % 2 === 0 ? 'bg-white' : 'bg-theme-50'"
              >
                <td class="whitespace-nowrap px-6 py-4 text-sm font-medium">
                  {{ field.property }}
                </td>

                <td class="w-32 whitespace-nowrap px-6 py-4 text-xs">
                  {{ field.type }}
                </td>
                <td class="px-6 py-4 text-xs">
                  {{ field.description }}
                  <span v-if="field.mandate" class="text-theme-500 italic">({{ field.mandate }})</span>
                  <div v-if="field.keys" class="mt-2 flex">
                    <span class="mr-2 font-semibold">Fields:</span>
                    <div class="">
                      <code
                        v-for="(k, ii) in field.keys"
                        :key="ii"
                        class="text-theme-700 mr-1 mb-1 text-xs"
                      >{{ k }}</code>
                    </div>
                  </div>
                </td>
