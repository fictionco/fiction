...
> Testing your application is part of the web development. Factor.js helps you to make it as easy as possible.
...
import { Factor, Builder } from 'factor'
import { resolve } from 'path'

// We keep a reference to Factor so we can close
// the server at the end of the test
let factor = null

// Init Factor.js and start listening on localhost:4000
test.before('Init Factor.js', async t => {
  const rootDir = resolve(__dirname, '..')
  let config = {}
  try { config = require(resolve(rootDir, 'factor.config.js')) } catch (e) {}
  config.rootDir = rootDir // project folder
  config.dev = false // production build
  config.mode = 'universal' // Isomorphic application
  factor = new Factor(config)
  await new Builder(factor).build()
  factor.listen(4000, 'localhost')
})

// Example of testing only generated html
test('Route / exits and render HTML', async t => {
  let context = {}
  const { html } = await factor.renderRoute('/', context)
  t.true(html.includes('<h1 class="red">Hello world!</h1>'))
})

// Example of testing via DOM checking
test('Route / exits and render HTML with CSS applied', async t => {
  const window = await factor.renderAndGetWindow('http://localhost:4000/')
...
You can add ESLint with Prettier pretty easily with Factor.js, first, you need to add the npm dependencies:
...
It is also recommended to enable ESLint hot reloading mode via webpack. This way ESLint will run on save during `npm run dev`. Just add the following to your `factor.config.js`:
