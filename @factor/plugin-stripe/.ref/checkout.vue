<script lang="ts">
import { currentUser, emitEvent, setting, stored } from '@factor/api'
import { factorBtn, factorIcon, factorInputText, factorLink } from '@factor/ui'
import {
  getProductConfig,
  getStripeClient,
  requestCoupon,
...
export default {
  components: {
    ...
    FactorLink: factorLink,
    FactorInputText: factorInputText,
    ...
    <div class="checkout">
      <div v-if="!status" class="header">
        ...
        <div class="action">
          ...
          <FactorLink v-else :query="{ ...$route.query, interval: `month` }">
            Pay Monthly Instead
          </FactorLink>
          <FactorLink path="/plans">
            Change plan?
          </FactorLink>
        </div>
      </div>
      <div v-if="status === 'success'" class="status-info">
        <div class="success">
          ...
          <div class="sub">
            <p>
              Congratulations! You've officially upgraded to
              {{ productConfig.title }}
            </p>
          </div>
          <div class="action">
            <FactorLink btn="primary" path="/docs/pro-suite">
              Using {{ productConfig.title }} &rarr;
            </FactorLink>
            <FactorLink btn="default" path="/dashboard/developer">
              Developer Dashboard &rarr;
            </FactorLink>
          </div>
        </div>
      </div>
      <div v-else class="checkout-info">
        <div class="checkout-col payment-details">
          ...
          <div class="purchasing">
            ...
            <div class="product">
              <div class="name">
                {{ productConfig.title || "No Title" }}
              </div>
              <div class="sub">
                {{ productConfig.description || "No Description" }}
              </div>
            </div>
          </div>
          <ul>
            <li>
              <FactorIcon icon="fas fa-check" />
        ...
        <div class="checkout-col payment-info">
          <div class="payment-area">
            ...
            <div class="item terms">
              By accepting the order and purchasing a subscription, you're
              agreeing to the
              <FactorLink path="/terms-of-service">
                terms of service
              </FactorLink>.
            </div>
            ...
            <div class="item coupon">
              <div
                v-if="!toggleCode"
                class="label"
                @click="toggleCode = true"
              >
                Add Code
              </div>
              <div v-if="toggleCode" class="coupon-code">
                <FactorInputText v-model="coupon" placeholder="Enter Code" />
                <FactorBtn
                  btn="default"
                  size="small"
