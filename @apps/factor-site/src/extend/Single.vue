    ...
    <section class="splash">
      <div class="ext-content-area header">
        ...
        <div class="ext-content-pad header-pad">
          <div class="content">
            <router-link class="back" :to="`/${extensionType}s`">
              <span>
                &larr; Back to
            ...
            <div class="text">
              <div class="title-wrap">
                ...
                <h3 class="description" v-text="getContent(post.description)" />
                <div class="actions">
                  <a btn="primary" @click="scrollTo(`#install`)">
                    Add to Project &darr;
                  </a>
                  <router-link
                    v-if="post.demo"
            ...
            <div class="info" :class="block">
              <div v-if="block == 'install'" class="install-extension">
                ...
                <div class="move-to-project">
                  <div
                    class="install-code"
                    @click="copyPackageName(post.packageName)"
                  >
              ...
              <div v-else-if="block == 'meta'" class="meta">
                <div class="meta-grid">
                  <div
                    v-for="(meta, ii) in metaItems"
                    :key="ii"
                    class="meta-item"
                  >
                    <div class="title">{{ meta.name }}</div>
                    <router-link v-if="meta.link" class="link" :to="meta.value">
                      {{ meta.link }}
                    </router-link>
...
export default {
  components: {
    ElemCta,
    //CallToAction: defineAsyncComponent(() => import("./ElemCta.vue")),
    //TextEntry: defineAsyncComponent(() => import("../el/Entry.vue")),
  },
  data() {
    return {
      getData: "",
  ...
  methods: {
    ...
    screenshotStyle(index: number) {
      const style = {
        transform: `translate(${index * 15}%, ${index}%) scale(${
          1 - index * 0.1
        })`,
        zIndex: 50 - index,
      }
