...
describe('serverRenderHtml', () => {
  ...
  it('should construct the correct HTML when valid inputs are provided', async () => {
    ...
    // Assert the results
    expect(html.replace(testUtils.fictionApp.port.toString(), 'PORT')).toMatchInlineSnapshot(`"<!DOCTYPE html><html bar="foo"><head><headtags></headtags><meta name="generator" content="Fiction 6.0.11"></head><body foo="bar"><bodytagsopen></bodytagsopen><div id="app"><htmlbody></htmlbody></div><bodytags></bodytags><!--{"renderedPathname":"/","isProd":true}--></body></html>"`)
