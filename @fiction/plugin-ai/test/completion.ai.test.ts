...
describe('ai completions', async () => {
  ...
  const openaiApiKey = testUtils.factorEnv.var('OPENAI_API_KEY')
  ...
  // const _factorUnsplash = new FactorUnsplash({ ...testUtils, unsplashAccessKey })
  const factorAi = new FactorAi({ ...testUtils, factorMedia, openaiApiKey })

  const baseInstruction = webDesignBasePrompt

  it('gets a completion', async () => {
    const r2 = await factorAi.queries.AiCompletion.serve({
      _action: 'completion',
      baseInstruction,
      objectives: {
    ...
    const r4 = await factorAi.queries.AiCompletion.serve({
      _action: 'completion',
      baseInstruction,
      objectives: {
