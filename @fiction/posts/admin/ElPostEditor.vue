...
const viewModes = [
  { label: 'compose', icon: 'i-tabler-edit' },
  { label: 'audience', icon: 'i-tabler-users' },
  { label: 'email', icon: 'i-tabler-mail' },
  { label: 'web', icon: 'i-tabler-world' },
] as const

const activeViewMode = vue.ref('compose')
...
vue.onMounted(() => {
  vue.watch(() => activeViewMode.value, (v) => {
    if (v === 'compose') {
      postEditController.hideContextDrawer.value = false
    }
    else {
      postEditController.hideContextDrawer.value = true
    }
  }, { immediate: true })
})
</script>
      ...
      >
        <div class="flex items-center gap-2 justify-center">
          <XButton
            ...
            respond="icon:xl"
            design="outline"
            :class="activeViewMode === mode.label ? '' : 'opacity-80'"
            :theme="activeViewMode === mode.label ? 'green' : 'default'"
            :icon="mode.icon"
            size="xs"

            @click.stop="activeViewMode = mode.label"
          >
            {{ toLabel(mode.label) }}
          </XButton>
        </div>
        ...
        <div class="flex items-center gap-2">
          ...
          >
            <XButton
              rounding="full"
              :icon="postEditController.hideContextDrawer.value ? 'i-tabler-arrow-bar-left' : 'i-tabler-arrow-bar-right'"
              size="sm"
              respond="icon:xl"
              @click.prevent="postEditController.hideContextDrawer.value = !postEditController.hideContextDrawer.value"
            />
          </ElTooltip>
        </div>
      </div>
      <div v-if="activeViewMode === 'compose'" class="flex-grow flex flex-col gap-4 h-full min-h-0">
