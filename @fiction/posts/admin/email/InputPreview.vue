...
import type { FictionPosts } from '../../index'
...
const { modelValue } = defineProps<{ modelValue?: TablePostConfig, card: Card }>()

const _emit = defineEmits<{
  (event: 'update:modelValue', payload: TablePostConfig): void
}>()
    ...
    <template v-else-if="emailConfig">
      <div>
        <ElInput
          ...
          input="InputSelect"
          :list="[
            { value: 'dark' }, { value: 'light' }, { label: 'User Default', value: '' }]"
        />
      </div>
