...
function toggleDocsNav(): void {
  if (vis.value) {
    ...
    resetUi({ scope: 'all', cause: 'docsNav' })
    vis.value = true
  }
}
...
<template>
  <PageWrap2 class="">
    <div class="text-theme-900 bg-white px-4 py-24 md:px-8">
      <div class="text-theme-900 mx-auto w-full max-w-screen-xl bg-white">
        <div class="lg:flex">
          <div
          ...
          <div
            ...
            :class="
              vis
                ? 'block h-screen absolute top-16 z-50 lg:hidden p-2'
                : 'hidden'
            "
          >
            <div
                ...
                <ul>
                  <template v-for="(group, i) in groups" :key="i">
                    <li v-if="group.title" class="mt-4">
                      ...
                      <ul class="group-items">
                        <li v-for="(docId, ii) in group.menu" :key="ii">
                          <router-link
                            class="hover:text-primary-500 flex items-center px-3 py-2"
                            :to="docRoute(docId)"
                            :class="
                              isCurrentNav(docId)
                                ? 'text-primary-500'
                    ...
                    <template v-else>
                      <li v-for="(docId, ii) in group.menu" :key="ii">
                        <a
                          class="hover:text-primary-500 text-theme-500 flex items-center px-3 py-2 transition-colors duration-200"
                          :href="docRoute(docId)"
                        >
                          {{ docTitle(docId) }}
                        </a>
