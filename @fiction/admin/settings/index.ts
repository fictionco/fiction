...
export function getOrgSettings(args: { org?: Organization }) {
  ...
  return {
    essentials: createOption({
      ...
      options: [
        createOption({ schema, key: `name`, label: 'Name', input: 'InputText', placeholder: 'Enter a name', isRequired: true, description: 'Your brand name' }),
        createOption({ schema, key: 'email', label: 'Email', input: 'InputEmail', isRequired: true, description: 'Your primary contact email.' }),
        createOption({ schema, key: 'handle', label: 'Handle', input: 'InputHandle', placeholder: 'my-handle', props: { table: 'fiction_org', columns: [{ name: 'handle' }] }, description: 'Used in URLs and mentions.' }),
        createOption({ schema, key: 'profile.headline', label: 'Headline', input: 'InputUrl', isRequired: true, placeholder: 'Enter a headline' }),
        createOption({ schema, key: 'profile.summary', label: 'About', input: 'InputTextarea', isRequired: true, placeholder: 'Enter a description' }),
        createOption({ schema, key: 'avatar', label: 'Avatar', input: 'InputMedia' }),
        createOption({ schema, key: 'branding.logo', label: 'Logo', subLabel: 'For visual identity', input: 'InputMedia' }),
        createOption({ schema, key: 'branding.primaryColor', label: 'Primary Color', input: 'InputColorTheme', placeholder: 'Default', props: { mode: 'bright' } }),
      ],
    }),
    domain: createOption({
      schema,
      ...
      options: [
        ...
        createOption({
          ...
          input: vue.defineAsyncComponent(() => import('@fiction/site/plugin-builder/tools/CustomDomain.vue')),
          isRequired: true,
        }),
        createOption({
          key: 'domainSetupInstructions',
          label: 'Setup Instructions',
          input: vue.defineAsyncComponent(() => import('@fiction/site/plugin-builder/tools/CustomDomainInstructions.vue')),
          props: {
