...
import type { ActionItem, FictionUser } from '@fiction/core'
...
const itemId = vue.computed(() => fictionRouter.params.value.itemId as 'login' | 'register' | undefined | '')
...
async function sendMagicLink(): Promise<void> {
  ...
  const r = fictionAdmin.emailActions.magicLoginEmailAction.requestSend({ to: email, fields: fields.value })
...
const config = vue.computed<TransactionProps | undefined>(() => {
  const mapping: Record<string, TransactionProps> = {
    register: {
      ...
      links: [{ name: 'Login Instead', onClick: () => updateItemItemId('login'), key: 'to-login' }],
    },

    login: {
      heading: 'Login',
      links: [{ name: 'Create Account Instead', onClick: () => updateItemItemId('register'), key: 'to-register' }],
    },
  }
    ...
    <template #links>
      <div class="text-sm text-theme-500 dark:text-theme-300 font-sans">
        <template v-if="itemId === 'register'">
          Already have an account? <a class="text-primary-500 dark:text-primary-400 hover:opacity-80" href="#" @click.prevent="updateItemItemId('login')">Sign in  &rarr;</a>
        </template>
        <template v-else-if="itemId === 'login'">
          First time here? <a class="text-primary-500 dark:text-primary-400 hover:opacity-80" href="#" @click.prevent="updateItemItemId('register')">Sign up for free &rarr;</a>
        </template>
      </div>
    </template>
