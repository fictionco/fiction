<script lang="ts">
import { standardDate, stored, toLabel } from '@factor/api'
import { dashboardBtn, dashboardListItem, factorLink, factorModal } from '@factor/ui'
...
export default {
  ...
  components: { DashboardListItem: dashboardListItem, DashboardBtn: dashboardBtn, FactorModal: factorModal, FactorLink: factorLink },
  data() {
    return {
      modalVisible: false,
      modalMode: '',
  ...
  <div>
    <div v-if="subscriptions.length === 0" class="zero-state banner">
      ...
      <div
        class="sub-title"
      >
        Experience pro extensions, pro features, dashboard enhancements and more.
      </div>
      <div class="actions">
        <FactorLink to="/plans" btn="primary">
          Pick a Plan &rarr;
        </FactorLink>
        <FactorLink to="/pro" btn="default">
          Learn More &rarr;
        </FactorLink>
      </div>
    </div>
    ...
    >
      <template #actions>
        ...
        >
          {{ item.cancel_at_period_end ? "Restore" : "Change" }} Plan
        </DashboardBtn>
        <DashboardBtn
    ...
    <FactorModal v-model:vis="modalVisible" class="sub-modal">
        ...
        <div class="info">
          ...
          <p>
            ...
            <router-link to="/contact">
              get in touch
            </router-link>&nbsp;with us and we'll do whatever we can to make you happy.
          </p>
        </div>
        <div class="actions">
          <DashboardBtn
          ...
          <DashboardBtn btn="default" @click="modalVisible = false">
            Close
          </DashboardBtn>
        </div>
      </template>
      <template v-else>
        ...
        <div class="info plans-list">
          <div
            v-for="({ plan, product }, i) in allPlans"
            :key="i"
          ...
          >
            ...
            <div class="actions">
              ...
              >
                {{ modalItem.cancel_at_period_end ? "Restore" : "Change to" }} this plan
              </DashboardBtn>
            </div>
          </div>
        </div>
