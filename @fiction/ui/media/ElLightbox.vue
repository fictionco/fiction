  ...
  <teleport to=".x-site">
    ...
    >
      ...
      >
        ...
        <div v-if="items && items.length > 1" class="absolute inset-y-0 left-0 flex items-start md:items-center py-4">
          <button class=" text-white p-4 rounded-r hover:opacity-75" @click.stop="previousItem">
            <div class="i-tabler-arrow-left text-4xl" />
          </button>
        </div>
        <div v-if="items && items.length > 1" class="absolute inset-y-0 right-0 flex items-start md:items-center py-4">
          <button class=" text-white p-4 rounded-l hover:opacity-75" @click.stop="nextItem">
            <div class="i-tabler-arrow-right text-4xl" />
          </button>
        </div>
        <AnimClipPath animate="expand">
          <div ref="animatedDivRef" class="relative h-[80dvh] w-[80dvw] flex flex-col overflow-hidden rounded-xl">
            <div class="grow relative" @click.stop>
              <ElImage ref="mediaRef" class="absolute inset-0" :media="activeItem.media" image-mode="contain" />
            </div>

            <div ref="contentRef" class=" space-y-2 text-white p-8 max-w-screen-sm mx-auto" @click.stop>
