...
<template>
  <ul role="list" class="space-y-4">
    <li v-for="(post, i) in posts" :key="i">
      <a :href="card.link(`/${i}`)" class="flex flex-wrap items-center justify-between gap-x-6 gap-y-4 py-5 sm:flex-nowrap hover:bg-theme-50 dark:hover:bg-theme-700 px-6 border border-theme-200/60 dark:border-theme-600 rounded-xl">
        <div>
          ...
          <div class="mt-1 flex items-center gap-x-2 text-xs leading-5 text-gray-500">
            <p>
              <a href="#" class="hover:underline">{{ post.desc }}</a>
            </p>
          </div>
        </div>
        <dl class="flex w-full flex-none justify-between gap-x-8 sm:w-auto items-center">
          <div class="flex -space-x-0.5">
            ...
            <dd v-for="(member, ii) in post.authors" :key="ii">
          ...
          <div class="hidden sm:flex sm:flex-col sm:items-end">
            ...
            <p class="mt-1 text-xs leading-5 text-theme-500">
              Created <time datetime="2023-01-23T22:34Z">{{ timeAgo('2023-01-23T22:34Z') }}</time>
            </p>
          </div>
