...
describe('subscriptione endpoint', async () => {
  ...
  const setTestData = dayjs('2024-06-11T00:00:00.000Z')
  ...
  it('has analytics data correct', async () => {
    ...
    await db(t.subscribe).insert([
      { publisher_id: orgId, user_id: userId, status: 'active', updated_at: setTestData.subtract(1, 'day').toDate() },
      { publisher_id: orgId, user_id: userId2, status: 'unsubscribed', previous_status: 'active', updated_at: setTestData.subtract(2, 'day').toDate() },
      { publisher_id: orgId, user_id: userId3, status: 'bounced', previous_status: 'active', updated_at: setTestData.subtract(3, 'day').toDate() },
      { publisher_id: orgId, user_id: userId4, status: 'active', updated_at: setTestData.subtract(5, 'hour').toDate() },
      { publisher_id: orgId, user_id: userId5, status: 'unsubscribed', previous_status: 'active', updated_at: setTestData.subtract(7, 'day').toDate() },
      { publisher_id: orgId, email: 'some@email.com', status: 'active', updated_at: setTestData.subtract(4, 'day').toDate() },
    ])

    const r = await testUtils.fictionSubscribe.queries.SubscriptionAnalytics.serve({
      _action: 'subscriptionAnalytics',
      publisherId: orgId,
...
describe('refineTimelineData', () => {
  it('correctly refines timeline data with daily interval', () => {
    const timeStartAt = dayjs('2024-06-01T00:00:00.000Z')
    const timeEndAt = dayjs('2024-06-11T00:00:00.000Z')
    const timeZone = 'UTC'
    const interval: TimeLineInterval = 'day'
    const now = dayjs('2024-06-11T00:00:00.000Z')
    ...
    expect(refinedData).toEqual([
      { date: 'rollup', subscriptions: 3, unsubscribes: 2, cleaned: 1, label: 'Totals', tense: 'past' },
  ...
  it('handles empty data array', () => {
    ...
    const refinedData = refineTimelineData({
      ...
      interval,
      data,
      now: dayjs('2024-06-11T00:00:00.000Z'),
      withRollup: true,
    })
  ...
  it('correctly refines timeline data with hourly interval', () => {
    const timeStartAt = dayjs('2024-06-10T00:00:00.000Z')
    const timeEndAt = dayjs('2024-06-10T12:00:00.000Z')
    const timeZone = 'UTC'
    ...
    const refinedData = refineTimelineData({
      ...
      interval,
      data,
      now: dayjs('2024-06-10T12:00:00.000Z'),
      withRollup: false,
    })
