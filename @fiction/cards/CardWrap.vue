...
const colorScheme = vue.computed(() => {
  const parts = [standardUc.value?.scheme?.base]

  if (isLightMode.value)
    parts.push(standardUc.value?.scheme?.light)
...
const containerStyle = vue.computed(() => {
  ...
  const spacing = standardUc.value?.spacing || {}
  ...
  style['--content-width'] = spacing.contentWidth ? `var(--spacing-${spacing.contentWidth})` : '100%'
  style['--vertical-spacing'] = spacing.verticalSpacing ? `var(--spacing-${spacing.verticalSpacing})` : '0'
...
<template>
  ...
  >
    ...
    <ElImage class="object-cover w-full h-full absolute inset-0" />
  </div>
</template>
