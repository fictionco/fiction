...
<template>
  <div v-if="!loading" :data-value="JSON.stringify({ subscribed })" :data-wrap-mode="uc.presentationMode">
    <template v-if="uc.presentationMode !== 'inline'">
      <teleport v-if="uc.presentationMode === 'onLoad'" to=".x-site">
        <transition
          enter-active-class="ease-[cubic-bezier(0.25,1,0.33,1)] duration-1000"
          enter-from-class="opacity-0 scale-125"
          enter-to-class="opacity-100"
          leave-active-class="ease-[cubic-bezier(0.25,1,0.33,1)] duration-1000"
          leave-from-class="opacity-100"
          leave-to-class="opacity-0 scale-125"
        >
          <div v-if="showCard && !dismissedLoad" :data-mode="uc.presentationMode" class=" pointer-events-none z-[45] text-theme-800 dark:text-theme-0 fixed left-0 top-0 flex h-full w-full items-center justify-center bg-theme-0 dark:bg-theme-900">
