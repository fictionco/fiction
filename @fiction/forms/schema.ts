...
export const formConfigCols = [
  ...
  new Col({ key: 'ownerId', sec: 'permanent', sch: () => z.string().min(1), make: ({ s, col }) => s.string(col.k) }),
  ...
  new Col({ key: 'layout', sec: 'setting', sch: () => FormLayout, make: ({ s, col }) => s.jsonb(col.k) }),
...
const FormFieldValue = z.union([z.string(), z.number(), z.boolean(), z.array(z.string()), z.array(z.number()), z.record(z.unknown()), z.null()])
...
export const formSubmissionCols = [
  ...
  new Col({ key: 'results', sec: 'setting', sch: () => z.record(z.unknown()), make: ({ s, col }) => s.jsonb(col.k).notNullable() }),
